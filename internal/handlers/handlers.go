...
func SetupRoutes(app *fiber.App, argocdClient *argocdclient.Clientset, argocdNamespaces, argocdComponentsToIgnore, argocdKnownClusters []string, offlineToken string) {
	// Register all APIs here - just add one line per API
	health.RegisterRoutes(app)

	// Register ArgoCD API if client is available
	if argocdClient != nil {
		argocdHandler, err := argocd.NewHandler(argocdClient, argocdNamespaces, argocdComponentsToIgnore, argocdKnownClusters, offlineToken)
		if err != nil {
